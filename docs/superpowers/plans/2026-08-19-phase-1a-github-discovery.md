...
- [x] **Step 1: Write failing tests** for valid defaults, empty repositories, missing files, and malformed `owner/name` identifiers using literal temporary TOML documents.
- [x] **Step 2: Verify RED** with `python -m unittest tests.test_config -v`; imports must fail because `ranger.config` does not exist.
- [x] **Step 3: Implement the minimum config loader** with `tomllib.load`, immutable dataclasses, and direct validation. Raise `ConfigError` with user-facing messages.
- [x] **Step 4: Verify GREEN** with `PYTHONPATH=src python -m unittest tests.test_config -v`.
...
- [x] **Step 1: Write failing tests** whose fake process results contain complete representative `gh` JSON for repository metadata and issue fields. Cover successful parsing, missing `gh`, invalid authentication, command failure, and malformed JSON.
- [x] **Step 2: Verify RED** with `PYTHONPATH=src python -m unittest tests.test_github -v`; imports must fail because `ranger.github` does not exist.
- [x] **Step 3: Implement the boundary** using `subprocess.run(..., capture_output=True, text=True, check=False)` for:
...
- [x] **Step 4: Verify GREEN** with `PYTHONPATH=src python -m unittest tests.test_github -v`.
...
- [x] **Step 1: Write failing CLI tests** for text output, JSON output, command-line repository overrides, the no-work message, and concise config/GitHub errors. Assert on observable stdout, stderr, and return values rather than mock call counts.
- [x] **Step 2: Verify RED** with `PYTHONPATH=src python -m unittest tests.test_cli -v`; imports must fail because `ranger.cli` does not exist.
- [x] **Step 3: Implement minimal coordination**: parse `run`, resolve `~/.config/ranger/config.toml`, apply overrides, validate auth once, discover every repository, sort results, and render. Both console and module entry points must raise `SystemExit(main())`.
- [x] **Step 4: Verify GREEN** with `PYTHONPATH=src python -m unittest tests.test_cli -v` and then `PYTHONPATH=src python -m unittest discover -s tests -v`.
...
- [x] **Step 1: Document `uv tool install .` and `pipx install .`, `gh auth login`, the exact TOML schema, CLI overrides, JSON output, and Phase 1A's read-only boundary.**
...
- [x] **Step 3: Install the built wheel into a temporary isolated environment** and run `ranger --help`, `ranger run --help`, and `python -m ranger --help`.
- [x] **Step 4: Inspect `git diff --check`, the staged diff, and a secret-pattern scan before committing the self-contained Phase 1A slice.**
